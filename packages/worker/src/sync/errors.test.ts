...
test('worker exposes frontend sync APIs but no legacy manage routes', async () => {
  ...
  assert.ok(source.includes("app.get('/api/check/:id'"), 'sync check page route remains available')
  assert.equal(source.includes("app.get('/api/sync/operations/:id'"), false)
...
test('sync operation logs are scoped by opaque id and exclude tokens', async () => {
  ...
  assert.match(source, /headers\.set\('X-Sync-Operation-Url', `\/api\/check\/\$\{id\}`\)/)
  assert.match(source, /renderSyncOperationHtml\(operation\)/)
